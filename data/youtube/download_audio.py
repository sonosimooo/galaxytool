...
def print_title():
    Write.Print(r'''
 _______  ___ ___  ______    ___  _______    ______    _______  ___ ___  ______   ___      _______  _______  ______   
|   _   ||   Y   ||   _  \  |   ||   _   |  |   _  \  |   _   ||   Y   ||   _  \ |   |    |   _   ||   _   ||   _  \  
|.  1   ||.  |   ||.  |   \ |.  ||.  |   |  |.  |   \ |.  |   ||.  |   ||.  |   ||.  |    |.  |   ||.  1   ||.  |   \ 
|.  _   ||.  |   ||.  |    \|.  ||.  |   |  |.  |    \|.  |   ||. / \  ||.  |   ||.  |___ |.  |   ||.  _   ||.  |    \
|:  |   ||:  1   ||:  1    /|:  ||:  1   |  |:  1    /|:  1   ||:      ||:  |   ||:  1   ||:  1   ||:  |   ||:  1    /
|::.|:. ||::.. . ||::.. . / |::.||::.. . |  |::.. . / |::.. . ||::.|:. ||::.|   ||::.. . ||::.. . ||::.|:. ||::.. . / 
...
def scarica_audio(link):
    try:
        options = {
            'format': 'bestaudio/best',  # Scarica solo l'audio
            'outtmpl': os.path.join(output_folder, '%(title)s.%(ext)s'),  # Percorso di salvataggio
            'quiet': True,  # Evita troppi log
            'no_warnings': True  # Disabilita i warning
        }
        with YoutubeDL(options) as ydl:
            ydl.download([link])
            Write.Print(f"The audio has been downloaded successfully!", Colors.white_to_green)
            time.sleep(1.5)
            os.system("cls")
...
def main():
    ...
    Write.Print(f'''
                ...
                ''', Colors.black_to_red)
    Write.Print(f"\n| {username} | >> Download Audio? (y/n):", Colors.black_to_red)
    yn = input()
    if yn == "y":
